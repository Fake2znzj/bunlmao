...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Antares Python Wrapper - Port 26270 + Ngrok supported")
    ...
    parser.add_argument("--port", type=int, default=None, help="Set dashboard port (e.g. --port 26270) - overrides env and config")
    parser.add_argument("--set-port", type=int, default=None, help="Set port and save to config.json then run (e.g. --set-port 26270)")
    parser.add_argument("--ngrok", action="store_true", help="Enable ngrok tunnel to expose dashboard publicly (bypass firewall)")
    parser.add_argument("--ngrok-token", type=str, default=None, help="Ngrok authtoken (or set NGROK_AUTHTOKEN env)")
    parser.add_argument("--ngrok-region", type=str, default="ap", help="Ngrok region: ap (Asia), us, eu, au, sa, jp, in (default ap)")
    ...
    if ngrok_enabled:
        rprint("[cyan]🌐 Ngrok enabled - starting tunnel...[/cyan]")
        try:
            from src.python.ngrok_helper import start_ngrok_tunnel, download_ngrok_binary
            # Try download binary if pyngrok not available
            if not ngrok_token:
                rprint("[yellow]⚠ NGROK_AUTHTOKEN chưa set - ngrok free sẽ giới hạn, nên set token từ https://dashboard.ngrok.com/get-started/your-authtoken[/yellow]")
            # Start tunnel
            ngrok_public_url = start_ngrok_tunnel(port, authtoken=ngrok_token, region=ngrok_region)
            if ngrok_public_url:
                rprint(f"[green]✅ Ngrok URL: {ngrok_public_url}[/green]")
            else:
                rprint("[yellow]⚠ Ngrok tunnel chưa chạy được, thử tải binary...[/yellow]")
                bin_path = download_ngrok_binary()
                if bin_path:
                    rprint(f"[dim]Đã tải ngrok binary: {bin_path}, thử lại...[/dim]")
                    ngrok_public_url = start_ngrok_tunnel(port, authtoken=ngrok_token, region=ngrok_region)
        except Exception as e:
            rprint(f"[red]Ngrok start lỗi: {e}[/red]")
            import traceback
            traceback.print_exc()
